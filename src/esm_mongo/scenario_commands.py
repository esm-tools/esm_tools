...
def ls():
    ...
    scenarios = document_models.Scenario.objects
    logger.debug(scenarios)
    return scenarios

def mk(field_attrs={}, interactive=False):
    connect("esm_tools", host=auth_commands.HOST)
    known_scenarios = [scen.name for scen in document_models.Scenario.objects]
    available_scenarios = known_scenarios + ["None of these, please help me to make a new one"] # Fix with db hookup
    if interactive:
        chosen = questionary.select("The following scenarios already exist, do you maybe want to use one of these?", choices=available_scenarios).ask()
    ...
    if interactive:
        new_scenario = document_models.Scenario.mk_interactively()
    else:
        new_scenario = document_models.Scenario(**field_attrs)
...
def show(name):
    connect("esm_tools", host=auth_commands.HOST)
    scens = document_models.Scenario.objects(name=name)
    for scen in scens:
