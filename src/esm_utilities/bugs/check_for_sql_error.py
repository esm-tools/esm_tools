#!/usr/bin/env python3
"""
Detects the SQLAlchemy I/O errors.

Summary
=======
This is a small program that checks a given run for SQL I/O errors.
...
An example usage::
    ...
    # The default usage:
    $ ./check_for_sql_error.py <my_experiment>
    Checking experiment T21G21O21_001...
    Year: 2068
    Run has crashed due to SQL I/O errors.

    $ echo $?
    1

    Checking a specific year:
    $ ./check_for_sql_error.py -y 2067 <my_experiment>
    Checking experiment T21G21O21_01...
    Year: 2067
    No SQL I/O problems detected.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Dectects if a run encountered a SQL I/O error."
    )
...
def look_for_sql_error(args, log):
    sql_errors = 0
    decode_errors = 0
    with open(f"{args.path}/scripts/{log}", "rb") as log:
        for line in log.readlines():
            try:
                line = line.decode()
                if "sqlalchemy.exc.OperationalError" in line:
                    sql_errors += 1
    ...
    if not args.quiet:
        if sql_errors > 0:
            print(f"Run has crashed due to SQL I/O errors.")
        else:
            print("No SQL I/O problems detected.")
        if decode_errors > 0:
