...
def get_file_components(config):
    """Components that participate in file operations (excludes 'general')."""
    return [m for m in esm_parser.get_components(config) if m != "general"]
...
def rename_sources_to_targets(config):
    ...
    for filetype in config["general"]["all_model_filetypes"]:
        for model in get_file_components(config):
...
def compute_and_log_file_checksums(config):
    ...
    for component in get_file_components(config):
        component_files = {}
        for filetype in filetypes:
