def prepare_environment(config):
    ...
    environment_dict = {
        ...
        "ice_bedrock_change_file": (
            config["general"]["experiment_couple_dir"] + "/bedrock_change.nc"
        ),
        ...
        "pism_solidearth_initialize_method": config["pism"][
            "solidearth_initialize_method"
        ],
        "pism_solidearth_initialize_dummyrun_file": config["pism"].get(
            "solidearth_initialize_dummyrun_file", ""
        ),
    ...
    if config["general"]["run_number"] > 1:
        environment_dict["latest_restart_file_pism"] = config["pism"][
            "restart_in_targets"
        ]["restart"]
