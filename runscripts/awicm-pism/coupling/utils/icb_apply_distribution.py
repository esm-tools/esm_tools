...
def read_pism_file(ifile):
    ...
    data = fl.tendency_of_ice_amount_due_to_discharge.where(
        fl.tendency_of_ice_amount_due_to_discharge < pism_trunc_fkt * N_min
    )
    ...
    lon_bnds = fl.lon_bnds.where(
        fl.tendency_of_ice_amount_due_to_discharge < pism_trunc_fkt * N_min, drop=True
    )
            ...
            for i, (s, d) in enumerate(zip(Nlength, sdepth)):
                ...
                if list(sbin):
                    ...
                    for i in range(len(l)):
                        ...
                        tmp_x = (
                            (1 - np.sqrt(r1)) * p1.x
                            + (np.sqrt(r1) * (1 - r2)) * p2.x
                            + (r2 * np.sqrt(r1)) * p3.x
                        )
                        tmp_y = (
                            (1 - np.sqrt(r1)) * p1.y
                            + (np.sqrt(r1) * (1 - r2)) * p2.y
                            + (r2 * np.sqrt(r1)) * p3.y
                        )
...
def find_FESOM_elem(nod2d_file, elem2d_file, lons, lats):
    points = []
    nod2d = pd.read_csv(
        nod2d_file, header=0, names=["lon", "lat", "coastal"], sep="\s+", index_col=0
    )
    elem2d = pd.read_csv(
        elem2d_file, header=0, names=["nod1", "nod2", "nod3"], sep="\s+"
    )
    ...
    with tqdm(total=len(lons), file=sys.stdout, desc="find FESOM elements") as pbar:
        for lon, lat in zip(lons, lats):
            tmp, ind = PointTriangle_distance(
                lon,
                lat,
                np.array(lon1),
                np.array(lat1),
...
def _back_rotate(self):
    ...
    with open(self.nod_file, "r") as csvfile:
        nodes = pd.read_csv(
            csvfile, header=None, sep=r"\s* \s*", skiprows=1, engine="python"
        )
        nodes.drop(nodes.index[0], inplace=True)
        nodes.columns = ["index", "lon", "lat", "mask"]
        [lon_tmp, lat_tmp] = r2g_v(
            np.array(nodes["lon"].values[:], dtype=float).transpose(),
