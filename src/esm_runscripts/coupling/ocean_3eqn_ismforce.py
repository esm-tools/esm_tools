...
def parse_arguments():
    ...
    #
    parser.add_argument("--OCEAN_INPUT_FILE", "-O", "-Oce", nargs="?", required=True, \
                        help="Ocean input file (REQUIRED)")
    parser.add_argument("--ISM_INPUT_FILE", "-I", "-Ice", nargs="?", required=True, \
    ...
    #
    parser.add_argument("--OCEAN_TEMP_VARIABLE", "-T", nargs="?", default="theta_ocean", \
                        help="ocean temperature variable name")
    parser.add_argument("--OCEAN_SALT_VARIABLE", "-S", nargs="?", default="salinity_ocean", \
                        help="ocean salinity variable name")
    parser.add_argument("--OCEAN_DEPTH_VARIABLE", "-D", nargs="?", default="depth", \
                        help="ocean depth variable name")
    parser.add_argument("--OCEAN_VELOCITY_VARIABLE", "-V", nargs="?", default="Skip_Use_Of_Absolute_Velocity", \
                        help="ocean absolute velocity variable name (skip => do not consider velocity)")
    parser.add_argument("--OCEAN_TIME_VARIABLE", "-ot", nargs="?", default="time", \
    ...
    #
    parser.add_argument("--ISM_BED_VARIABLE", "-b", nargs="?", default="topg", \
                        help="ice model bedrock topography variable name")
    parser.add_argument("--ISM_MASK_VARIABLE", "-m", nargs="?", default="mask", \
                        help="ice model mask variable name")
    parser.add_argument("--ISM_THICK_VARIABLE", "-t", nargs="?", default="thk", \
                        help="ice model ice thickness variable name")
    parser.add_argument("--ISM_ICE_TEMP_VARIABLE", "-s", nargs="?", default="ice_surface_temp", \
                        help="ice model upper ice temperature variable name")
    parser.add_argument("--ISM_TIME_VARIABLE", "-it", nargs="?", default="time", \
                        help="ice model time variable name")

    #
    # Ice sheet input: values, where length and height are in "meter"
    #
    parser.add_argument("--ISM_MASK_FLOAT_VALUE", nargs="?", type=int, default=3, \
                        help="mask value of floating ice in ice model")
    parser.add_argument("--ISM_GRID_LENGTH", nargs="?", type=float, default=16000. , \
    ...
    parser.add_argument("--ISM_GRID_HEIGHT", nargs="?", type=float, default=40. , \
                        help="vertical grid size (meter) in ice model, between base and upper ice temperature (see input '-s', '--ISM_ICE_TEMP_VARIABLE'). If ISM_GRID_HEIGHT>0 use this value, if ISM_GRID_HEIGHT=0 use the ice thickness distribution, else use the difference between the two lowest layers") #basal grid layer
    parser.add_argument("--ISM_GRID_Z_AXIS_VARIABLE", nargs="?", default="z", \
                        help="variable name of vertical z-axis" ) #z-axis name

    #
    # Output file name
    #
    parser.add_argument("--OUTPUT_FILE", nargs="?", default='Shelfmelt_forcing.nc4', \
                        help="name of ice sheet forcing output file")
    parser.add_argument("--LOGGING_OUTPUT_FILE", "-l", nargs="?", default='ocean_hydrography2ism_forcing.log', \
                        help="name of logging output file")
    parser.add_argument("--LOGGING_OUTPUT_LEVEL", "-v", nargs="?", default='INFO', \
                            help="logging level", choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL', 'NONE'])

    #
    # Output file name: variables
    #
    parser.add_argument("--OUT_BASAL_TEMP_VARIABLE", nargs="?", default="shelfbtemp", \
                        help="output file, basal temperature variable name")
    parser.add_argument("--OUT_BASAL_MASSFLUX_VARIABLE", nargs="?", default="shelfbmassflux", \
                        help="output file, basal mass flux variable name")
    parser.add_argument("--OUT_BASAL_HEATFLUX_VARIABLE", nargs="?", default="shelfbheatflux", \
                        help="output file, basal heat flux variable name")

    #
    # Output: What shall be computed?
    #
    parser.add_argument("--OUT_CALC_BASAL_MELT", nargs="?", type=bool, default=True, \
                        help="Shall we compute the basal melting rate?", choices=[True, False])
    parser.add_argument("--OUT_CALC_BASAL_TEMP", nargs="?", type=bool, default=True, \
                        help="Shall we compute the basal ice temperature rate?", choices=[True, False])
    parser.add_argument("--OUT_CALC_FRONTAL_MELT", nargs="?", type=bool, default=False, \
                        help="Shall we compute the frontal melting rate?", choices=[True, False])
    parser.add_argument("--OUT_CALC_FRONTAL_MELT_AT_ICE_POSITION", nargs="?", type=bool, default=True, \
                        help="Frontal melting shall be relased on the ice shelf side (True) or in the neihboring open ocean (False)?", choices=[True, False])
    parser.add_argument("--OUT_CALC_MERGE_BASAL_FRONTAL_MELT", nargs="?", type=bool, default=False, \
                        help="Shall we merge basal and vertical integrated frontal melt into the basal melting rate (bmelt=bmelt+fmelt)?", choices=[True, False])
    parser.add_argument("--OUT_CALC_LESS_BASAL_LOW_CAVITY", nargs="?", type=bool, default=False, \
                        help="Shall we reduce the basal melting rate for low vertical space between bedrock and basal ice interface?", choices=[True, False])
    ...
    #
    parser.add_argument("--OUT_WRITE_2DFIELDS", "-2", nargs="?", type=bool, default=True, \
                        help="Shall we write 2Dim output fields?", choices=[True, False])
    parser.add_argument("--OUT_WRITE_3DFIELDS", "-3", nargs="?", type=bool, default=False, \
                        help="Shall we write 3Dim output fields?", choices=[True, False])
    parser.add_argument("--OUT_WRITE_TEMP_EXCESS", "-E", nargs="?", type=bool, default=False, \
                        help="Shall we compute and write the temperature excess above local melting temperature (CMIP6 thermal forcing)?", choices=[True, False])
    parser.add_argument("--OUT_WRITE_AUXVAR", "-A", nargs="?", type=bool, default=False, \
                        help="Shall we write auxillary output fields?", choices=[True, False])
    parser.add_argument("--OUT_WRITE_MASS_FLUX_KG_BASED", "-K", nargs="?", type=bool, default=True, \
                        help="Shall mass flux be based on the unit kg/m2/s?", choices=[True, False])
    parser.add_argument("--OUT_WRITE_MACHINE_INFORMATION", nargs="?", type=bool, default=True, \
                        help="Shall we write some basic information about the use machine and user", choices=[True, False])

    #
    # Misc switches
    # Note densities are in "kg m^-3" and pressure uses "Pa"
    #
    parser.add_argument("--LOGGING_WRITE_TIME", nargs="?", type=bool, default=True, \
                        help="Shall we write time to standard output?", choices=[True, False])
    parser.add_argument("--DENSITY_ICE", nargs="?", type=float, default=910., \
                        help="density of ice")
    parser.add_argument("--DENSITY_OCEAN", nargs="?", type=float, default=1026., \
                        help="density of ocean water")
    parser.add_argument("--DENSITY_FRESHWATER", nargs="?", type=float, default=1000., \
                        help="density of fresh water")
    parser.add_argument("--SEALEVEL_PRESSURE", nargs="?", type=float, default=101325.0, \
                        help="sealevel pressure")
    parser.add_argument("--AIR_TEMPERATURE", nargs="?", type=float, default=-20.0, \
...
logging.basicConfig(filename=args.LOGGING_OUTPUT_FILE, filemode='w', \
    format='%(asctime)s:%(levelname)-8s:%(message)s',
                    level=loglevel)
print("Start logging for current job into file '"+args.LOGGING_OUTPUT_FILE+"' with level:"+str(loglevel))
logging.debug("**** Start logging for current job into file '%s' with level '%s' ****", \
             args.LOGGING_OUTPUT_FILE, loglevel)
...
#
if ( args.OUT_CALC_BASAL_MELT == None ) : args.OUT_CALC_BASAL_MELT = True
if ( args.OUT_CALC_BASAL_TEMP == None ) : args.OUT_CALC_BASAL_TEMP = True
if ( args.OUT_CALC_FRONTAL_MELT == None ) : args.OUT_CALC_FRONTAL_MELT = True
if ( args.OUT_CALC_FRONTAL_MELT_AT_ICE_POSITION == None ) : args.OUT_CALC_FRONTAL_MELT_AT_ICE_POSITION = True
if ( args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT == None ) : args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT = True
if ( args.OUT_CALC_LESS_BASAL_LOW_CAVITY == None ) : args.OUT_CALC_LESS_BASAL_LOW_CAVITY = True
if ( args.OUT_WRITE_2DFIELDS == None ) : args.OUT_WRITE_2DFIELDS = True
if ( args.OUT_WRITE_3DFIELDS== None ) : args.OUT_WRITE_3DFIELDS = True
if ( args.OUT_WRITE_TEMP_EXCESS == None ) : args.OUT_WRITE_TEMP_EXCESS = True
if ( args.OUT_WRITE_AUXVAR == None ) : args.OUT_WRITE_AUXVAR = True
if ( args.OUT_WRITE_MASS_FLUX_KG_BASED == None ) : args.OUT_WRITE_MASS_FLUX_KG_BASED = True
if ( args.OUT_WRITE_MACHINE_INFORMATION == None ) : args.OUT_WRITE_MACHINE_INFORMATION = True
if ( args.LOGGING_WRITE_TIME == None ) : args.LOGGING_WRITE_TIME = True

if ( args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT == True and args.OUT_CALC_FRONTAL_MELT == False ):
    logging.warning("Merging basal and frontal melting is TRUE while calculation of frontal melt is FALSE")
if ( args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT == True and args.OUT_CALC_BASAL_MELT == False ) :
    logging.warning("Merging basal and frontal melting is TRUE while calculation of basal melt is FALSE")
...
if ( args.OUT_CALC_FRONTAL_MELT == False and args.OUT_CALC_BASAL_MELT == False and args.OUT_CALC_BASAL_MELT == False ) :
    logging.error("Basal temperature, basal and frontral melting is FALSE => Nothing to do??")
    sys.exit(3)


if ( args.DENSITY_ICE <= 0. ) :
    logging.error("Unphysical density of ice, DENSITY_ICE=%f", args.DENSITY_ICE)
if ( args.DENSITY_OCEAN<= 0. ) :
    logging.error("Unphysical density of ocean water, DENSITY_OCEAN=%f", args.DENSITY_OCEAN)
if ( args.DENSITY_FRESHWATER <=0. ) :
    logging.error("Unphysical density of fresh water, DENSITY_FRESHWATER=%f", args.DENSITY_FRESHWATER)
if ( args.SEALEVEL_PRESSURE <=0. ) :
    logging.error("Unphysical sea level pressure, SEALEVEL_PRESSURE=%f", args.SEALEVEL_PRESSURE)
...
if ( args.ISM_BED_VARIABLE == None ) : ierror = ierror + 1
if ( args.ISM_THICK_VARIABLE == None ) : ierror = ierror + 1
if ( args.ISM_MASK_VARIABLE == None ) : ierror = ierror + 1
if ( args.ISM_ICE_TEMP_VARIABLE == None ) : ierror = ierror + 1
if ( args.ISM_TIME_VARIABLE == None ) : ierror = ierror + 1
if ( args.ISM_MASK_FLOAT_VALUE == None ) : ierror = ierror + 1
if ( args.ISM_GRID_LENGTH == None ) : ierror = ierror + 1
if ( args.ISM_GRID_HEIGHT == None ) : ierror = ierror + 1
if ( args.ISM_GRID_Z_AXIS_VARIABLE == None ) : ierror = ierror + 1
if ( args.OUTPUT_FILE == None ) :
    ierror = ierror + 1
    logging.error("--OUTPUT_FILE is set without value")
if ( args.LOGGING_OUTPUT_FILE == None ) : ierror = ierror + 1
if ( args.LOGGING_OUTPUT_LEVEL == None ) : ierror = ierror + 1
if ( args.OUT_BASAL_TEMP_VARIABLE == None ) : ierror = ierror + 1
if ( args.OUT_BASAL_MASSFLUX_VARIABLE == None ) : ierror = ierror + 1
if ( args.OUT_BASAL_HEATFLUX_VARIABLE == None ) : ierror = ierror + 1
if ( args.DENSITY_ICE == None ) : ierror = ierror + 1
if ( args.DENSITY_OCEAN == None ) : ierror = ierror + 1
if ( args.DENSITY_FRESHWATER == None ) : ierror = ierror +1 
if ( args.SEALEVEL_PRESSURE == None ) : ierror = ierror + 1
if ( args.AIR_TEMPERATURE == None ) : ierror = ierror + 1
...
Depth2Press = dens_ocean * gravitional_acceleration  # water depth in water pressure (Pa/m)
...
def boundaryFluxSaltTempW3(dz0_ice, Tice, dpres, T, S,  \
                           gammaS=5.05E-7, gammaT=1.0E-4, \
                           rho_oce=1026.0, rho_ice=910.0):
    ...
    eta4 = b + c*dpres;          # Using here c<0 => +c*dpres
    ...
    if (False):
        ...
        TmpVar= np.where(flag == False, np.sqrt(TmpVar), 0.)
        Sb1 = eta5*0.5 - TmpVar
        Sb2 = eta5*0.5 + TmpVar
        Sb  = np.where(flag == True, Smelt0, \
                       np.where(Sb1<=0, Sb2, (Sb1 + Sb2)*0.5))
    else :
        Sb1 = np.zeros_like(S)
...
if ( flag_use_xarray ) :
    ...
    try:
        ...
        logging.warning("MISSING mask variable '%s' use instead commen value of %i",args.ISM_MASK_VARIABLE,args.ISM_MASK_FLOAT_VALUE)
    try:
        thk = fileice.get(args.ISM_THICK_VARIABLE).isel(time=-1).data
    except:
        thk = np.full_like(topg, 1000.)
        logging.warning("MISSING thickness variable '%s' use instead commen value of 1000.0",args.ISM_THICK_VARIABLE)
    try:
        icetemp_top = fileice.get(args.ISM_ICE_TEMP_VARIABLE).isel(time=-1).data
    except:
        icetemp_top = np.full_like(topg, args.AIR_TEMPERATURE)
        logging.warning("MISSING ice temperature variable '%s' use instead commen value of %f",args.ISM_TEMP_VARIABLE,args.AIR_TEMPERATURE)
...
if ( args.ISM_GRID_HEIGHT > 0 ) :
    ...
    logging.info("  * dz (height diff.) = %s m : command line parameter <= 'ISM_GRID_HEIGHT>0'",'{:7.2f}'.format(dz0_ice))
elif ( args.ISM_GRID_HEIGHT == 0 ) :
    dz0_ice = thk
    logging.info("  * dz (height diff.) = %s m : ice thickness data <='ISM_GRID_HEIGHT=0'", \
    ...
    logging.info("  * dz (height diff.) = %s m : from vertical axis %s <= 'ISM_GRID_HEIGHT<0'", \
                 '{:7.2f}'.format(dz0_ice), args.ISM_GRID_Z_AXIS_VARIABLE)

if ( args.ISM_GRID_LENGTH > 0 ) :
    dx0_ice = args.ISM_GRID_LENGTH # x-direction (meter)
    dy0_ice = args.ISM_GRID_LENGTH # y-direction (meter)
    logging.info("  * dx&dy (horizontal size) = %s m/%s m : command line argument <= 'ISM_GRID_LENGTH>0'", \
                 '{:8.1f}'.format(dx0_ice), '{:8.1f}'.format(dy0_ice) )
else :
    dx0_ice = abs(xpism[1] - xpism[0])
    dy0_ice = abs(ypism[1] - ypism[0])
    logging.info("  * dx&dy (horizontal size) = %s m/%s m : vertical axis %s and %s <= 'ISM_GRID_LENGTH<0'", \
                 '{:8.1f}'.format(dx0_ice), '{:8.1f}'.format(dy0_ice), 'x', 'y')
...
if ( flag_use_xarray ) :
    ...
    try:
        ...
        logging.debug("  - Time does not exists, use dummy fime %f",'{:12.4f}'.format(time_oce))
...
if ( flag_use_uabs ) :
    logging.warning("Use of the ocean velocity '%s' is not yet implemented", \
...
isocean         = mask >= 3 # ocean ; #isocean     = np.logical_or(mask == 7, mask == 4,  mask == 3)
...
floating_free      = np.where(isfloating, floating_basedepth - topg, \
...
if ( args.OUT_CALC_LESS_BASAL_LOW_CAVITY ) :
    bmb_frac = np.where(isice, np.minimum(1.0, np.maximum(0.0,  df_a * floating_free + df_b)), 1.0)
...
for i0 in range(ilen) :
    for j0 in range(jlen) :
        ...
        if isfloating[i0, j0] :
            [kbelow[i0, j0], frac_below[i0, j0]] = find_kindex(depth, floating_basedepth[i0, j0])
...
#
if ( args.LOGGING_WRITE_TIME ) :
    print("FREEZING temperature through the column     ("+time.strftime('%H:%M:%S [%Z]')+")")
else :
...
if ( args.OUT_WRITE_TEMP_EXCESS ) :
    if ( args.LOGGING_WRITE_TIME ) :
        print("TEMPERATURE above melting in the column     ("+time.strftime('%H:%M:%S [%Z]')+")")
    else :
...
for it in range( tlen ) :
    ...
    logging.info("  Time step %5i with time stamp %s", \
                 it, str(time_oce[it]))
    ...
    if ( args.OUT_CALC_BASAL_TEMP or args.OUT_CALC_BASAL_MELT ):
        if ( args.LOGGING_WRITE_TIME ) :
            print("     BASAL freezing temperature    ("+time.strftime('%H:%M:%S [%Z]')+")")
        else :
        ...
        for i0 in range(ilen) :
            for j0 in range(jlen) :
                TempIceBase[it, i0, j0] = TempIceBase3[it, kbelow[i0, j0], i0, j0] \
                                          * frac_below[i0, j0] \
                                          + TempIceBase3[it, kabove[i0, j0], i0, j0] \
                                          * (1.0-frac_below[i0, j0])

    if ( np.logical_or(args.OUT_CALC_BASAL_MELT, args.OUT_CALC_FRONTAL_MELT) ) :
        if ( args.LOGGING_WRITE_TIME ) :
            print("     MELTING through the column    ("+time.strftime('%H:%M:%S [%Z]')+")")
        else :
            print("     MELTING through the column")
        
        for k0 in range(klen) :
            print('      depth layer ('+str(k0)+' / '+str(klen-1)+') = '+str(depth[k0])+'m')
            returnall = np.array(boundaryFluxSaltTempW3(dz0_ice, icetemp_top[:, :], \
                                                        press3[k0, :, :],  \
                                                        temp[it, k0, :, :], \
                                                        salt[it, k0, :, :] ))
    ...
    if ( args.OUT_CALC_BASAL_MELT ):
        if ( args.LOGGING_WRITE_TIME ) :
            ...
            print("     BASAL melting")
        for i0 in range(ilen) :
            procent=100.*float(i0/ilen)
            string2show = str('{:5.1f}'.format(procent))+'%'
            print('{0}\r'.format(string2show), end='')

            for j0 in range(jlen) :
                if ( isfloating[i0, j0] ):
                    bmelt[it, i0, j0] = bmelt3[it, kbelow[i0, j0], i0, j0] \
                                        * frac_below[i0, j0] \
                                        + bmelt3[it, kabove[i0, j0], i0, j0] \
                                        * (1.0-frac_below[i0, j0])
                    SFlux[it, i0, j0] = SFlux3[it, kbelow[i0, j0], i0, j0] \
                                        * frac_below[i0, j0] \
                                        + SFlux3[it, kabove[i0, j0], i0, j0] \
                                        * (1.0-frac_below[i0, j0])
                    TFlux[it, i0, j0] = TFlux3[it, kbelow[i0, j0], i0, j0] \
                                        * frac_below[i0, j0] \
                                        + TFlux3[it, kabove[i0, j0], i0, j0] \
                                        * (1.0-frac_below[i0, j0])
                    SaltBound[it, i0, j0] = SaltBound3[it, kbelow[i0, j0], i0, j0] \
                                            * frac_below[i0, j0] \
                                            + SaltBound3[it, kabove[i0, j0], i0, j0] \
                                            * (1.0-frac_below[i0, j0])                    
                    TempBound[it, i0, j0] = TempBound3[it, kbelow[i0, j0], i0, j0] \
                                            * frac_below[i0, j0] \
                                            + TempBound3[it, kabove[i0, j0], i0, j0] \
                                            * (1.0-frac_below[i0, j0])
                ...
                for i0 in range(1, ilen-1) :
                    for j0 in range(1, jlen-1) :
                        if ( isfrontalmelt[i0, j0] ) :
                            for ii in [i0-1, i0+1] : # left and right: ii=i0+-1, jj=j0
                                if ( isfloating[ii, j0] ) : # do melt
                                    ##iice=ii ; jice=j0 # At the floating ice position
                                    if ( args.OUT_CALC_FRONTAL_MELT_AT_ICE_POSITION ) :
                                        iice=ii ; jice=j0 # At the floating ice position
                                    else :
                                        iice=i0 ; jice=j0 # At the ocean/ice front position

                                    fmelt3[it, k0, iice, jice] = \
                                        fmelt3[it, k0, iice, jice] + bmelt3[it, k0, i0, j0]
                                    fmelt3_rate[it, k0, iice, jice] = \
                                        fmelt3_rate[it, k0, iice, jice] + \
                                        bmelt3[it, k0, i0, j0] \
                                        *dz_oce[k0]*dy0_ice
                            for jj in [j0-1, j0+1] : # above and below : ii=i0, jj=j0+-1
                                if ( isfloating[i0, jj] ) : # do melt
                                    ...
                                    fmelt3_rate[it, k0, iice, jice] = \
                                        ...
                                        bmelt3[it, k0, i0, j0] \
                                        *dz_oce[k0]*dx0_ice
                        fmelt[it, i0, j0] = fmelt[it, i0, j0] \
                            + fmelt3[it, k0, i0, j0]
                        fmelt_rate[it, i0, j0] = fmelt_rate[it, i0, j0] \
                            + fmelt3_rate[it, k0, i0, j0]
...
# ice sheet/shelf data
#
print(' Create ice-ocean forcing output data file  : ' +
      args.OUTPUT_FILE)
logging.info("Create ice-ocean forcing output file : '%s'", \
             args.OUTPUT_FILE)
...
# lon, lat
lon_var     = filebmb.createVariable('longitude', 'd', ('x', 'y', ))
lon_var.long_name   = 'longitude'
lon_var.units       = 'degree'
lon_var.valid_range = [-180.0, 360.0]
lat_var     = filebmb.createVariable('latitude', 'd', ('x', 'y', ))
lat_var.long_name   = 'latitude'
lat_var.units       = 'degree'
lat_var.valid_range = [-90.0, 90.0]

area_var     = filebmb.createVariable('area', 'd', ('x', 'y', ))
...
# Ocean data grid
# kbelow
kbelow_var    = filebmb.createVariable('kbelow','i',('x','y', ))
...
# basal interface
basedepth_var    = filebmb.createVariable('basedepth','d',('x','y', ))
...
# bmelt fraction
bmelt_frac_var       = filebmb.createVariable('bmb_frac','d',('x','y', ))
...
if ( args.OUT_CALC_LESS_BASAL_LOW_CAVITY ) :
    ...
    bmelt_frac_var.use_fractional_basal_melting_doc = "Since OUT_CALC_LESS_BASAL_LOW_CAVITY="+str(args.OUT_CALC_LESS_BASAL_LOW_CAVITY)+", we apply fractional basal melting for low vertical space between bedrock and ice shelf base. Pleases inspect also variable 'floating_free'"
    bmelt_frac_var.depth_free_start = depth_free_start
    bmelt_frac_var.depth_free_start_doc = "At and below this free vertical start depth="+str(depth_free_start)+"m, the fraction is zero, Between start and end depth the fraction is linearly interpolated"
    bmelt_frac_var.depth_free_end     = depth_free_end
    bmelt_frac_var.depth_free_end_doc = "At and below this free vertical end depth="+str(depth_free_end)+"m, the fraction is one. Between start and end depth the fraction is linearly interpolated"
else :
    bmelt_frac_var.use_fractional_basal_melting = "No, False"
    bmelt_frac_var.use_fractional_basal_melting_doc = "Since OUT_CALC_LESS_BASAL_LOW_CAVITY="+str(args.OUT_CALC_LESS_BASAL_LOW_CAVITY)+", we do not use fractional basal melting for low vertical space between bedrock and ice shelf base"

bmelt_frac_var.floating_space_of_grounded=floating_space_grounded
bmelt_frac_var.floating_space_of_grounded_doc="Grounded ice has an assumed vertical free space of "+str(floating_space_grounded)+" meter"


if ( args.OUT_WRITE_AUXVAR ) :
    # isocean ice mask
    isocean_var    = filebmb.createVariable('isocean','i',('x','y', ))
    isocean_var.long_name   ='isocean_mask (1: receives frontal melt, 0: else)'
    isocean_var.coordinates ='latitude longitude'

    # isfloating ice mask
    isfloat_var    = filebmb.createVariable('isfloating','i',('x','y', ))
    ...
    isfrontalmelt_var    = filebmb.createVariable('isfrontalmelting','i',('x','y', ))
    isfrontalmelt_var.long_name   ='isfrontalmelting_mask (1: receives frontal melt, 0: else)'
    isfrontalmelt_var.coordinates ='latitude longitude'

    # floating_free
    float_free_var       = filebmb.createVariable('floating_free','d',('x','y', ))
    ...
    # depthbelow
    depthbelow_var    = filebmb.createVariable('depthbelow','d',('x','y', ))
...
#
bit_var = filebmb.createVariable(args.OUT_BASAL_TEMP_VARIABLE,'d',('time', 'x','y', ))
...
#
# basal melting
bmbR_var = filebmb.createVariable(args.OUT_BASAL_MASSFLUX_VARIABLE,'d',('time', 'x','y', ))
...
# frontal melting
fmbR_var = filebmb.createVariable('fmbI','d',('time', 'x','y', ))
...
if ( args.OUT_CALC_FRONTAL_MELT ) :
    fmb3R_var = filebmb.createVariable('fmb','d',('time', 'z', 'x','y',  ))
...
dTdz_var    = filebmb.createVariable('dTdz','d',('time', 'x','y', ))
...
if ( args.OUT_WRITE_2DFIELDS ):
    ...
    #
    # basal melting
    bmb_var = filebmb.createVariable('bmb_unit_area','d',('time', 'x','y', ))
    if ( args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT):
    ...
    # salt flux
    sflux_var = filebmb.createVariable('Sflux_unit_area','d',('time', 'x','y', ))
    ...
    # temperature flux
    tflux_var = filebmb.createVariable('Tflux_unit_area','d',('time', 'x','y', ))
    ...
    sbound_var = filebmb.createVariable('basal_salt_boundary','d',('time', 'x','y', ))
    ...
    tbound_var = filebmb.createVariable('basal_temp_boundary','d',('time', 'x','y', ))
    ...
    #
    # frontal melting
    fmb_var = filebmb.createVariable('fmbI_unit_area','d',('time', 'x','y', ))
...
if ( args.OUT_WRITE_3DFIELDS ):
    # basal melting
    bmb3R_var = filebmb.createVariable(args.OUT_BASAL_MASSFLUX_VARIABLE+'3D','d',('time', 'z', 'x','y', ))
    if ( args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT):
        bmb3R_var.long_name   ='basal and frontal melting rate combined at each ocean level '
    else :
    ...
    bmb3_var = filebmb.createVariable(args.OUT_BASAL_MASSFLUX_VARIABLE+'3D_unit_area','d',('time', 'z', 'x','y', ))
    if ( args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT):
        bmb3_var.long_name   ='basal and frontal melting rate combined at each ocean level per unit area'
    else :
    ...
    fmb3_var = filebmb.createVariable('fmb_unit_area','d',('time', 'z', 'x','y',  ))
    ...
    # salt flux
    sflux3_var = filebmb.createVariable('Sflux_unit_area3D','d',('time', 'z', 'x','y',  ))
    ...
    # temperature flux
    tflux3_var = filebmb.createVariable('Tflux_unit_area3D','d',('time', 'z', 'x','y',  ))
    ...
    # boundary layer salinity
    sbound3_var = filebmb.createVariable('salt_boundary3D','d',('time', 'z', 'x','y',  ))
    ...
    # boundary layer temperature
    tbound3_var = filebmb.createVariable('temp_boundary3D','d',('time', 'z', 'x','y',  ))
...
if ( args.OUT_WRITE_TEMP_EXCESS ) :
    tabove3_var = filebmb.createVariable('temp_above_melting3D','d',('time', 'z', 'x','y',  ))
    tabove3_var.long_name    ='ocean temperature excess above in local refreezing point through water column'
...
if ( args.OUT_WRITE_3DFIELDS ):
    ...
    if ( np.logical_and(args.OUT_CALC_FRONTAL_MELT, args.OUT_CALC_MERGE_BASAL_FRONTAL_MELT) ):
        if ( args.OUT_WRITE_MASS_FLUX_KG_BASED ) :
...
#
if ( args.LOGGING_WRITE_TIME ) :
    print("Close all open files and flush data buffer(s)    ("+time.strftime('%H:%M:%S [%Z]')+")")
else :
