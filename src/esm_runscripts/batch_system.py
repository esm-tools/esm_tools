...
# kh 22.06.22 adjust total number of MPI processes via mpi_num_groups at lowest level (nproc)
    ...
    def get_run_commands(config, subjob, batch_or_shell):  # here or in compute.py?
        ...
        if subjob.startswith("compute"):
            ...
            if batch_or_shell == "SimulationSetup":
                ...
                # add phase
                commands.append(f"--phase {subjob}")
                # add task
                commands.append(f"-t run_phase")
                # add date
    ...
    def write_run_batch_script(config, cluster, batch_or_shell="batch"):
        ...
        phases = workflow.clusters[cluster]["phases"]
        ...
        with open(runfilename, "w") as runfile:
            ...
            for phase in phases:
                ...
                commands = batch_system.get_run_commands(
                    config, phase["name"], batch_or_shell
                )
...
#                if cluster in reserved_jobtypes:
            ...
            if not config["general"]["next_date"] >= config["general"]["final_date"]:
                ...
                expid = config["general"].get("expid", None)
                scriptname = config["general"].get("scriptname", None)
                current_date = config["general"].get("current_date", None)
                current_date = current_date.format(form=9, givenph=False, givenpm=False, givenps=False)
