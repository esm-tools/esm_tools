...
def is_experiment_folder(checkpath):
    ...
    if "scripts" in os.listdir(checkpath) and \
            "bin" in os.listdir(checkpath) and \
...
def print_disclaimer():
    ...
    response = questionary.select(
            ...
            choices = ["Hell no, let me get out of here.", "Yes, I know what I am doing, and will be responsible for the outcome." ]
            ).ask()
...
def assert_question(question):
    response = questionary.select(
            question,
            choices = ["No, there is something wrong.", "Yes, that is exactly what I want.", "I want to quit."]
            ).ask()
    if "quit" in response:
        sys.exit(0)
    if "Yes" in response: 
...
def ask_for_action(folder):
    ...
    run_folders = [subfolder for subfolder in subfolders if subfolder.startswith("run_")]
    if len(run_folders) > 0:
        choices.insert(0, "Remove all run_* subfolders (recommended)")

    
    post_folders = [subfolder for subfolder in subfolders if subfolder.startswith("post_")]
...
def remove_subfolder(folder, saved_space):
    ...
    if "restart" in response or "bin" in response:
        print (f"Folder {response} should not be removed entirely, and esm_cleanup won't do it.")

    full_path = os.path.join(folder, response)
    if not assert_question(f"Do you really want to remove the whole subfolder {full_path}?"):
        return saved_space
