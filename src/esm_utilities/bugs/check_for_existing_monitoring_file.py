...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Dectects if a run encountered FileExistsError during monitoring."
    )
...
def look_for_monitoring_error(args, log):
    monitoring_errors = 0
    decode_errors = 0
    with open(f"{args.path}/scripts/{log}", "rb") as log:
        for line in log.readlines():
            try:
                ...
                continue
    if not args.quiet:
        if monitoring_errors > 0:
            print(f"Run has crashed due to esm-tools FileExistsError.")
            print(
                f"Consider removing the offending files in {args.path}/scripts/ and resubmitting"
            )
        else:
            print("No esm-tools FileExistsError detected.")
        if decode_errors > 0:
...
def main():
    ...
    if isinstance(log_file, str):
        if not args.quiet:
            print(f"Year: {year}")
        monitoring_errors = look_for_monitoring_error(args, log_file)
    elif isinstance(log_file, list):
        monitoring_errors = 0
        for thisyear, thislog in zip(year, log_file):
            if not args.quiet:
                print(f"Year: {thisyear}")
            monitoring_errors += look_for_monitoring_error(args, thislog)
    if monitoring_errors > 0:
        return 1
    return 0
