...
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt]
  \item \textbf{Skip the redundant pass-2 \code{construct+select}} in \code{fesom2ice}
        ($\sim$155\,s, free) --- same guard pattern as Fix~2, since the pass-2 \code{*\_var4ice.nc}
        \code{monmean} outputs are identical to pass~1.
  ...
  \item \code{couple\_in} \textbf{dEBM run ($\sim$65\,s)} --- the Fortran dEBM binary is
        \emph{recompiled every leg}; cache/skip the compile when the binary already exists.
...
\end{enumerate}
\emph{Net so far:} the \code{fesom2ice} handoff is $\sim$36\,min\,$\to\sim$8.5\,min (Fixes~1+2),
with a further $\sim$155\,s free once the pass-2 \code{monmean} is skipped.
