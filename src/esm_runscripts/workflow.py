...
class Workflow:
    ...
    def __init__(self, workflow_yaml):
        ...
        error = False
        self.clusters = workflow_yaml.get('clusters', None)
        self.default_cluster = workflow_yaml.get('default_cluster', None)

        # Initialize default workflow phases from defaults.yaml
        for phase in workflow_yaml.get('phases', []):
            self.phases.append(WorkflowPhase(workflow_yaml['phases'][phase]))
                ...
                if "phases" in w_config:
                    ...
                    for phase_name in w_config["phases"]:
                        ...
                        # if user phase (for each setup/model) has a non-default and unique name
                            ...
                            # set cluster of physe to default cluster if not set
                            phase_config["cluster"] = phase_config.get("cluster", self.default_cluster)
...
#                           #         esm_parser.user_error("ERROR", err_msg)
    ...
    def cluster_phases(self):
        ...
        - Appends new/user phases to cluster
        - If a cluster is not defined for a new phase, it wil be appended to the default cluster (sim_cluster)
        - If a cluster is defined for a new phase, it will be added to the workflow object
        """
        clusters = self.clusters
        # create an empty phases list for each new cluster
        #for cluster_name in self.get_phases_values_list("default", "cluster") + self.get_phases_values_list("user", "cluster"):
        for cluster_name in self.get_phases_values_list("user", "cluster"):
            if cluster_name not in clusters:
                clusters[cluster_name] = {"phases": []}

        # collect all new/user phases that are within the same cluster
        #for phase in self.phases + self.user_phases:
        # append user phases to their clusters and create new cluster
        #for phase in self.user_phases:
...
#        self.set_workflow_attrib("clusters", clusters)
    ...
    def order_phases_in_clusters(self):
        """
...
# correct workflow attributes (``last_task_in_queue``, ``first_task_in_queue``, ``next_run_triggered``)
        ...
        for cluster_name in clusters:
            ...
            user_phases = self.user_phases
            for user_phase in user_phases:
                user_run_after = user_phase.get("run_after", None)
                if user_run_after:
                    index_run_after = cluster["phases"].index(user_run_after)
                    print(len(cluster["phases"]))
                    if len(cluster["phases"]) > index_run_after+1:
                        if cluster["phases"][index_run_after+1] != user_phase["name"]:
                            cluster["phases"].insert(index_run_after+1, user_phase["name"])
                    else:
                        cluster["phases"].append(user_phase["name"])
            #old_next_triggered = cluster["next_run_triggered_by"]
...
#                phase3["next_submit"] = next_submits_phases[phase3["name"]]
    ...
    def get_workflow_commands_for_run(self, config):
        ...
        -------
            config: dict
        """
        phases = self.phases + self.user_phases
        phase_type = ""
        run_command = ""
        run_commands = []
    
        for phase in phases:
            phase_type = phase.get("phase_type", None)
...
def assemble_workflow(config):
    ...
    workflow = workflow.order_phases_in_clusters()
    ...
    # - This needs to be done before getting the run commands,
    #   because some function need workflow to be already set in config
    ...
    #        "workflow"
    #    ]["clusters"]["sim_cluster"].get("first_task_in_queue", None)
...
def init_default_workflow(config):
    ...
    if phases:
        ...
        # can be removed: default WorkflowPhase are already initialized in initialization of workflow object
        #for phase in phases:
